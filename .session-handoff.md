...
- `projects.py` (253 lines) - 7 methods: project operations (4 delegated, 3 pending)

**Main File Reduced**: 3,220 → 2,826 lines (check: `wc -l empirica/data/session_database.py`)
...
1. **ProjectRepository**: 3 methods need delegation
   - `create_project` (line ~2197)
   - `link_session_to_project` (line ~2245)
   - `aggregate_project_learning_deltas` (line ~2273)

2. **EpistemicRepository**: 21 methods (largest chunk) - NOT STARTED
   - All `*_assessment`, `*_vectors`, `store_*`, `get_*` methods
   - Check: `grep "def.*assessment\|def.*vectors" empirica/data/session_database.py`

3. **SessionRepository**: 10 methods - NOT STARTED
