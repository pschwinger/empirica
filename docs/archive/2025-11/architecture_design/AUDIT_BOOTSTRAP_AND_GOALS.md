...
## Issue 1: MCP Tool Name Mismatch ⚠️
...
## Issue 2: Goal Orchestrator Uses Placeholder (Heuristic Fallback) ⚠️
...
**What This Means:**
- `use_placeholder=True` → Uses `_placeholder_goal_generation()` method
...
CanonicalGoalOrchestrator(use_placeholder=True)
    ↓
orchestrate_goals() → _placeholder_goal_generation()
    ↓
Threshold-based logic:
  - if clarity < 0.60 → CLARIFY goal
  - if know < 0.70 → INVESTIGATE goal
  - if confidence >= 0.70 → ACT goal
    ↓
...
CanonicalGoalOrchestrator(llm_client=..., use_placeholder=False)
    ↓
orchestrate_goals() → Build meta-prompt
    ↓
LLM Client: Generate goals based on:
  ...
  - NO templates, NO thresholds
    ↓
Parse LLM response → Goals with genuine reasoning
```
...
- Low clarity? → Add "Clarify task" goal
- Low knowledge? → Add "Investigate" goal  
- High confidence? → Add "Execute" goal
...
# AFTER (accurate):
print("   ✅ Canonical goal orchestrator loaded (placeholder mode - threshold-based)")
print("   💡 For LLM-powered goal generation, configure llm_client")
```
...
print("   ✅ Goal orchestrator loaded (placeholder mode)")
print("   💡 Using threshold-based goal generation")
print("   💡 For LLM-powered goals, configure llm_client")
```
...
**Goal Orchestrator:**
- ✅ Architecture is sound (LLM-first design)
- ⚠️ Implementation uses placeholder (threshold-based)
- ❌ Not using genuine AI reasoning as intended

**Bootstrap:**
- ✅ Comments indicate awareness of heuristic issue
- ⚠️ Still initializes with `use_placeholder=True`
- ❌ Misleading comments claim "no heuristics"

**MCP Tool:**
- ✅ Bootstrap tool exists and works
- ⚠️ Name mismatch issue (`bootstrap_session` vs `empirica-bootstrap_session`)
